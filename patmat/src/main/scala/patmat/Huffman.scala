...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    //val distinctedList = chars.distinct

    def loop(distinctChars: List[Char], res: List[(Char, Int)]): List[(Char, Int)] =
      if (distinctChars.isEmpty) res
      else makeInner(distinctChars.head, chars) ++ loop(distinctChars.tail, res)

    loop(chars.distinct, List())

  }

  def makeInner(c: Char, list: List[Char]): List[(Char, Int)] = List((c, count(c, list)))

  def count(c: Char, list: List[Char]): Int =
    if (list.isEmpty) 0
    else if (!list.isEmpty && list.head == c) 1 + count(c, list.tail)
    else count(c, list.tail)
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def makeLeaf(freqsInner: List[(Char, Int)]): List[Leaf] =
      freqsInner match {
        case List() => List()
        case y::ys => Leaf(y._1,y._2) :: makeLeaf(ys)
      }
    ...
    def insert(x: Leaf, xs: List[Leaf]): List[Leaf] = xs match {
      case List() => List(x)
      case y :: ys => y
        if (x.weight <= y.weight) x :: xs else y :: insert(x, ys)
    }

    isort(makeLeaf(freqs))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case h::s::tail => (makeCodeTree(h,s)::tail).sortWith((t1,t2)=>weight(t1)<weight(t2))
  }
  ...
  /**
    ...
    */
  def until(f:List[CodeTree] => Boolean, g: List[CodeTree] => List[CodeTree])(tress: List[CodeTree]): List[CodeTree] =
    if (f(tress)) tress
    else until(f,g)(g(tress))
  /**
  ...
  def createCodeTree(chars: List[Char]): CodeTree =
  {
    ...
    val codeTree  = until(singleton,combine)(leafs)
    print(codeTree.head)
    codeTree.head
  ...
  def decode2(tree: CodeTree, bits: List[Bit]): List[Char] = tree match {
      case Leaf(c, _) if bits.isEmpty => List(c)
      case Leaf(c, _) => c :: decode2(tree, bits)
      case Fork(left, right, _, _) if bits.head == 0 => decode2(left, bits.tail)
